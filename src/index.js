...
$('#FindLocationHotspot').on('click', function (e) {
	...
	loc.forEach(function (l) {
		...
		if (t1 > -1 | t2 > -1) {
			...
			if (hotspot[mm[1]].locName == l.Location) {
				...
				pop += '<button role="button" class="btn btn-secondary OpenmanageLocations" val="\'' + l.checklists[0] + '\'"><i class="fas fa-edit"></i> Merge to/suggest as hotspot</button></p>'
	...
	locNoHot.forEach(function (l) {
		var td1 = l.Location + '<a href="#" class="OpenmanageLocations" val="\'' + l.checklists[0] + '\'"><i class="fas fa-edit pad-r"></i></a>';
		...
		$('#messages-table').append('<tr class="setview" lat=' + l.Latitude + ' lon=' + l.Longitude + '><td>' + td1 + '</td><td>' + td2 + '</td></tr>');
	})
});

$('.setview').on('click', function (e) {
	//console.log(e)
	//console.log(this)
	//map.setView(new L.LatLng(, ),16)
})

$('.OpenmanageLocations').on('click', function (e) {
	console.log(e)
	console.log(this)
	let subId = $(this).attr('val');
	console.log(subId)
...
var ListRegion = function (loc) {
	...
	country.forEach(function (c) {
		var html = '<div class="list-group-item "><a data-bs-target="#list-country-' + c.locID + '" class="list-crl-a" data-bs-toggle="collapse"><i class="fas fa-chevron fa-chevron-right"></i><span class="chevron-margin">' + c.name + '</span></a><a href="#" class="rcl-download" id="rcl-download-' + c.locID + '"><i class="fas fa-download" ></i></a><span class="badge bg-primary bg-pill">' + c.count.toString() + '</span></div>\
	...
	$('.rcl-download').on('click', function (e) {
		...
		if (!$(this).hasClass('fa-disabled')) {
			let val = $(this).hasClass('rcl-download') ? $(this).attr('id').replace('rcl-download-', '') : $(this.parentElement).attr('id').replace('rcl-download-', '')
